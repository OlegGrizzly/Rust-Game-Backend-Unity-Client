...
## Архитектура SDK (4 логических слоя)

**Текущее состояние**: пакет содержит шаблонные asmdef (`GameBackend`, `GameBackend.Editor`, `GameBackend.Tests`, `GameBackend.Editor.Tests`). Один `GameBackend.asmdef` используется до Phase 5.

**Стратегия .asmdef**: до Phase 5 — один `GameBackend.asmdef`, логическое разделение через папки (`Runtime/Core/`, `Runtime/Transport/`, `Runtime/Api/`, `Runtime/WebSocket/`). Физическое разделение на 4 .asmdef — в Phase 5 после стабилизации контрактов.

**Целевая архитектура (Phase 5+, 4 отдельных .asmdef):**
...
Опциональные модули (отдельные .asmdef с `defineConstraints`): `GameBackend.VContainer`, `GameBackend.R3`, `GameBackend.MessagePipe`.
...
Порядок реализации (7 фаз, детали в `ROADMAP.md`): Phase 0 (CORE: scaffolding) → Phase 1 (TEST+CORE: контракты) → Phase 2 (TEST→CORE→AUTH: transport + auth) → Phase 3 (TEST+REST: Account, Leaderboard) → Phase 4 (TEST+WS+REST: WebSocket + Chat, параллельно с Phase 3) → Phase 5 (CORE: split .asmdef + TEST+REST: 5 доменов) → Phase 6 (EDITOR: tools). TEST агент работает параллельно со всеми как драйвер TDD.
