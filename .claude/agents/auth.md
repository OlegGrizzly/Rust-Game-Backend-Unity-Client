...
- **AuthService** — 13 методов IAuthClient: Authenticate* (3 провайдера: username, email, device), Login* (3 провайдера), RefreshSession, Logout, ListSessions, RevokeSession, RevokeAllSessions, LinkProvider, UnlinkProvider
...
8. **Создать Example** в `com.gamebackend.sdk/Samples/Auth/AuthExample.cs`:
   - MonoBehaviour с `[ContextMenu]` для КАЖДОГО метода IAuthClient (AuthenticateUsername, AuthenticateEmail, AuthenticateDevice, LoginUsername, LoginEmail, LoginDevice, RefreshSession, Logout, ListSessions, RevokeSession, RevokeAllSessions, LinkProvider, UnlinkProvider)
